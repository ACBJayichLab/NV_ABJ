...
class RabiAnalysis:
    ...
    @classmethod
    def importing_matlab_output_rabi_data(cls,folder_path, basename):
        """
        This imports the data following a standard file formatting from the matlab output
        """
        #Dictionaries for storing data pulled from multiple files eventually this is turned into a pandas dataframe so that it can be efficiently worked on
        data_norm_dict = {}
        data_signal_dict = {}
        data_ref_signal_dict = {}

        #This indicates when the first data file is open and we can take representative data such as two tau
        first_file = 0

        for f in listdir(folder_path):
            if basename in f and isfile(join(folder_path, f)):
                #saving a identifying number so the files are ordered appropriately in the dataframe
                file_number = int(f.split("_")[-1].replace(".txt",""))

                #importing the data into a pandas dataframe and then converting this into a dictionary
                data = pd.read_csv(os.path.join(folder_path,f), sep = "\t")
                data_norm_dict[file_number] = data["totalNorm"]
                data_signal_dict[file_number] = data["rawSignal"]
                data_ref_signal_dict[file_number] = data["rawRef"]

                #Getting common data like two tau length
                if first_file == 0:
                    two_tau = np.array(data["2Tau"]).astype("float")

        #basic math applied to the dataframe (average and the error bars are calculated)
        norm_data = pd.DataFrame(data_norm_dict)
        ref_data = pd.DataFrame(data_ref_signal_dict)
        signal_data = pd.DataFrame(data_signal_dict)

        return cls(two_tau = two_tau,raw_signal = signal_data,raw_ref = ref_data,normalized_data = norm_data)
