...
class CWESR(MeasurementSequence):
    ...
    def generate_sequence(self,readout_time_s:float,readout_trigger_duration_s:float,
        ...
        """This sequence is simply the runs a trigger with a delay time where the rf is on"""
        seq = Sequence()
        # seq.add_step(np.round((readout_trigger_duration_s)*1e9),                 [rf_trigger,laser_trigger])
    ...
    def experimental_setup(self, rf_source:MicrowaveSource, frequency_list_hz:npt.NDArray, rf_amplitude_dbm:npt.NDArray, number_of_data_taking_cycles:int)->int:
        ...
        return number_of_data_taking_cycles
        
    
    def counts_to_raw_counts(self, data:tuple)->npt.NDArray:
