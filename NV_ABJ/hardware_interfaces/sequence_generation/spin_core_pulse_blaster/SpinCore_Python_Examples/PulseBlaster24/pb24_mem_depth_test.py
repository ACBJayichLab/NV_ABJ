...
if argc < 2:
    print("Usage:")
    print("\t %s <clock speed in MHz> <number of instructions to write>" % sys.argv[0])
    print("\t %s <number of instructions to write>" % sys.argv[0])
    pause()
    exit(2)
...
i = 0
while i < n_inst//2:
    pb_inst_pbonly(0x000000, CONTINUE, 0, dell * ns)
    i += 1
while i < n_inst-1:
    pb_inst_pbonly(0xFFFFFF, CONTINUE, 0, dell * ns)
    i += 1
pb_inst_pbonly(0xFFFFFF, BRANCH, 0, dell * ns)
...
print("Output should have period of %d ns." % (dell*n_inst))
